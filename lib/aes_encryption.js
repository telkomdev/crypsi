...
const encrypt = (alg, key, data) => {
    ...
    let cipher = createCipheriv(alg, keyBuf, nonceBuf);
...
const decrypt = (alg, key, data) => {
    ...
    let decipher = createDecipheriv(alg, keyBuf, nonceBuf);
    ...
    if (SUPPORTED_AUTH_TAG_MODES.includes(metaAlg.mode)) {
        ...
        let encryptedTextUtf8 = buf.subarray(0, sFrom);
        encryptedText = encryptedTextUtf8;
    } else {
        encryptedText = buf;
    }
